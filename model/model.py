...
from .attention import GraphAttention, ScaledDotProductAttention, MultiHeadAttention
...
class AMER_Modi(BaseModel):
    ...
    def initialize(self, config, device):
        ...
        # 这里和模型的编码长度统一成256
        self.g_att_v = GraphAttention(config["visual"]["concept_size"], config)
        self.g_att_a = GraphAttention(config["audio"]["concept_size"], config)
        self.g_att_t = GraphAttention(config["text"]["concept_size"], config)
        #################
